...
export default function ProductSpecs({ 
  ...
  return (
    <div className="bg-gradient-to-r from-light-gold to-cream-gold border-r-4 border-classic-gold p-4 sm:p-6 mb-6 sm:mb-8 rounded-lg">
      <h3 className="text-heading-3 font-bold text-chocolate-brown mb-3 sm:mb-4">{title}</h3>
        ...
        {specs.map((spec, index) => {
          ...
          return (
            <div key={index} className="flex items-center space-x-2 space-x-reverse">
              ...
              <span className="text-traditional-brown font-medium text-body">{spec.text}</span>
            </div>
          );
        })}
      </div>
